-- Atualizar a função handle_new_user para incluir a página performance
...
BEGIN
  ...
  VALUES 
    ...
    (NEW.id, 'performance', true); -- Performance liberado por padrão
  
  RETURN NEW;
END;
$function$;

-- Adicionar permissão performance para usuários existentes
INSERT INTO public.user_page_permissions (user_id, page, can_access)
SELECT id, 'performance', true
FROM auth.users
WHERE id NOT IN (
  SELECT user_id 
