-- Adicionar nova página ao enum
ALTER TYPE page ADD VALUE 'ai_agents';
...
BEGIN
  ...
  VALUES 
    ...
    (NEW.id, 'ai_agents', true); -- AI Agents liberado por padrão
  
  RETURN NEW;
END;
$function$;

-- Adicionar permissão ai_agents para usuários existentes
INSERT INTO public.user_page_permissions (user_id, page, can_access)
SELECT id, 'ai_agents', true
FROM auth.users
WHERE id NOT IN (
  SELECT user_id 
  FROM public.user_page_permissions 
  WHERE page = 'ai_agents'
);